...
void main() {
  ...
  group("bilibili-client", () {
    ...
    setUp(() async {
      Hive.init('/hive');
      var b = await Hive.openBox('testBox');
      c = BilibiliClient(b);
      await c.init();
    });

    test('get-video-info', () async {
      String bvid = "1aE411n7wb";
      var res = await c.getVideoInfo(bvid);
      print(res);
      assert(res[0].contains(bvid));
      // print(res.body);
    });

    test('search', () async {
      var res = await c.search("祖娅纳惜");
      print(res[0]);
    });
